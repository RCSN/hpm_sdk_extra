...
static void ping_prepare_echo(struct netif *netif, struct icmp_echo_hdr *iecho, uint16_t len)
{
    size_t i;
    ...
    if (netif->name[0] == 'E' && netif->name[1] == 'X') {
...
static err_t ping_send(struct netif *netif, int s, ip_addr_t *addr, int size)
{
    int err;
    ...
    ping_prepare_echo(netif, iecho, (uint16_t) ping_size);
...
hpm_stat_t ping(struct netif *netif, char *target_name, uint32_t times, size_t size)
{
    ...
    while (1) {
        int elapsed_time;
        err = ping_send(netif, s, &target_addr, size);
        if (err == ERR_OK) {
