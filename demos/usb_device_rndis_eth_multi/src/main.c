...
#include "netconf.h"
#include "usbh_core.h"
#include "cdc_rndis_lwip.h"
#include "ping.h"
#include <stdio.h>
#include <string.h>

#ifndef IPERF_TEST_ENABLED
#define IPERF_TEST_ENABLED (0)
#endif

#if IPERF_TEST_ENABLED
...
#if IPERF_TEST_ENABLED
static bool iperf(struct netif *netif);
#endif

static void ping_or_iperf_task(void *pdata);
...
void main_task(void *pvParameters)
{
    ...
    xTaskCreate(ping_or_iperf_task, "ping_task", 2048, &gnetif, DEFAULT_THREAD_PRIO, NULL);
...
static void ping_or_iperf_task(void *pdata)
{
    ...
    while (1) {
        ...
        if (netif_is_up(netif[netif_index])) {
            if (strstr(netif_name[netif_index], "EX") != NULL) {
                dhcp_check = check_dhcp_success_status();
            } else {
            ...
            if (dhcp_check == true) {
#if IPERF_TEST_ENABLED
                netif_set_default(netif[netif_index]);
                while (1) {
                    if (iperf(netif[netif_index]) == true) {
                        break;
                    }
                    vTaskDelay(10);
                }
...
static bool select_mode(struct netif *netif, bool *server_mode, bool *tcp, enum lwiperf_client_type *client_type)
{
    ...
    while (1) {
        if (uart_check_status(BOARD_CONSOLE_UART_BASE, uart_stat_data_ready) == true) {
            code = uart_read_byte(BOARD_CONSOLE_UART_BASE);
            break;
        }
        vTaskDelay(100);
    }
...
static bool iperf(struct netif *netif)
{
    ...
    bool exit = false;
#if defined(LWIP_DHCP) && LWIP_DHCP
    if ((strstr(netif->name, "EX") != NULL) || (netif_dhcp_data(netif)->state == DHCP_STATE_BOUND)) {
#endif
        if (session == NULL) {
            session = start_iperf(netif);
        } else {
